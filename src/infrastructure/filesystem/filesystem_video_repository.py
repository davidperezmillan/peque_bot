...
class FilesystemVideoRepository(VideoRepository):
    ...
    async def download_video(self, video_message: VideoMessage, destination_dir: str) -> str:
        ...
        try:
            ...
            file_path = os.path.join(destination_dir, f"{video_message.document.id}.mp4")
            self.logger.info(f"Downloading video {video_message.document.id} to {file_path}")
            ...
            if os.path.exists(file_path):
                ...
                self.logger.info(f"Video {video_message.document.id} downloaded successfully. "
            ...
            self.logger.error(f"Failed to download video {video_message.document.id} to {destination_dir}: {str(e)}", exc_info=True)
            raise